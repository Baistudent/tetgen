...
static void get_density_region_center(const Bounds &b, REAL *center)
{
  center[0] = b.center[0];
  center[1] = b.center[1];
  center[2] = b.maxpt[2];
                                   ...
                                   const REAL *center)
{
  ...
  REAL pts[6][3] = {
      {center[0] - rx, center[1], center[2]},
      {center[0] + rx, center[1], center[2]},
      {center[0], center[1] - ry, center[2]},
      {center[0], center[1] + ry, center[2]},
      {center[0], center[1], center[2] + rz},
      {center[0], center[1], center[2] - rz}};
  for (int i = 0; i < 6; i++)
  {
                     ...
                     int region_type, const Bounds &b, tetgenio *custom_region)
{
  ...
  REAL region_center[3];
  get_density_region_center(b, region_center);

  REAL sizefactor = 0.20;
  REAL transition = 0.06 * b.span[2];
  if (region_type == 1)
  {
    REAL minpt[3] = {
        region_center[0] - 0.22 * b.span[0],
        region_center[1] - 0.22 * b.span[1],
        region_center[2] - 0.16 * b.span[2]};
    REAL maxpt[3] = {
        region_center[0] + 0.22 * b.span[0],
        region_center[1] + 0.22 * b.span[1],
        region_center[2] + 0.16 * b.span[2]};
  ...
  {
    REAL halfheight = 0.31 * b.span[2];
    REAL basept[3] = {region_center[0], region_center[1], region_center[2] - halfheight};
    REAL toppt[3] = {region_center[0], region_center[1], region_center[2] + halfheight};
  ...
  else if (region_type == 3)
  {
    ...
    in.add_density_region_sphere(region_center, radius, sizefactor, transition);
  }
...
int main()
{
  ...
  REAL halfheight = 0.31 * b.span[2];
  REAL cylbase[3] = {region_center[0], region_center[1], region_center[2] - halfheight};
  REAL cyltop[3] = {region_center[0], region_center[1], region_center[2] + halfheight};
  ...
  write_sphere_vtk(outdir + "/sphere_density_region.vtk",
                   region_center, sphereradius);

  tetgenio custom_region;
  make_custom_plc_region(custom_region, b, region_center);
  write_input_surface_vtk(outdir + "/custom_plc_density_region.vtk",
                          custom_region);

  if (!run_case(outdir, "baseline", 0, b, NULL))
    return 1;
  if (!run_case(outdir, "box_density", 1, b, NULL))
    return 1;
  if (!run_case(outdir, "cylinder_density", 2, b, NULL))
    return 1;
  if (!run_case(outdir, "sphere_density", 3, b, NULL))
    return 1;
  if (!run_case(outdir, "custom_plc_density", 4, b, &custom_region))
    return 1;
